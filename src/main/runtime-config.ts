...
export function resolveRegistry(override: string | undefined, fileText: string | undefined): RegistryResolution {
  ...
  if (override !== undefined && override.trim() !== '') {
    ...
    warning = `${REGISTRY_ENV_VAR} is not an http(s) URL; ignoring it`
  }
  ...
  if (fileText !== undefined) {
    ...
    try {
      ...
      return { url: DEFAULT_PLUGIN_REGISTRY, where: 'default', warning: warning ?? `${CONFIG_FILENAME} is not valid JSON (${detail})` }
    }
    const configured = (parsed as { pluginRegistry?: unknown } | null)?.pluginRegistry
    if (typeof configured === 'string' && configured.trim() !== '') {
      const url = normalizeRegistry(configured)
      if (url !== undefined) return { url, where: 'file', ...(warning === undefined ? {} : { warning }) }
      warning ??= `${CONFIG_FILENAME}: pluginRegistry is not an http(s) URL; ignoring it`
    } else if (configured !== undefined) {
      warning ??= `${CONFIG_FILENAME}: pluginRegistry must be a string; ignoring it`
    }
  }

  return { url: DEFAULT_PLUGIN_REGISTRY, where: 'default', ...(warning === undefined ? {} : { warning }) }
}
...
function resolution(): RegistryResolution {
  ...
  cached = resolveRegistry(process.env[REGISTRY_ENV_VAR], text)
